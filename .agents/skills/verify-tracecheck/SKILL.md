...
| `stand-in-provider.mjs` | `node $S/stand-in-provider.mjs [--port N] [--latency-ms N] [--fail-path REGEX]`, started with `hub` | A loopback System One stand-in with fixed latency; one JSON log line per request with its packet and in-flight count |
