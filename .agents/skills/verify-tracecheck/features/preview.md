...
- **Repository-local Git settings.** In the `division` fixture, run `git -C "$ROOT" config --unset core.hooksPath`, point `core.fsmonitor` at a script that touches a marker file, add a `post-index-change` hook in `$ROOT/.git/hooks` that touches another, and `touch "$ROOT"/src/*.ts` at least a second later so `git diff` rewrites the index. CLI `preview` and `review` and the MCP tools create neither marker. A `filter.<driver>.clean` command selected through `$ROOT/.git/info/attributes` still runs, as it does for `git status`.
- **Control characters.** In the `division` fixture, `git mv src/stats.ts "$(printf 'src/\033]0;t\007x.ts')"`, add `/* \033]52;c;eA==\007 */` inside the divisor's parentheses, and stage the change. Human `preview` and `review` output contain no raw control bytes (`LC_ALL=C tr -dc '\000-\010\013-\037\177' < FILE | wc -c` prints `0`) and show `\x1b]0;t\x07` instead. The Markdown report also backslash-escapes the brackets in the path. `preview --json` keeps the raw path.
