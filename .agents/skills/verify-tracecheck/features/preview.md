...
- `preview-gaps` reports omitted files, parse failures (by file and parser error code), budget limits, and heuristic-discovery limitations. Every file omitted for a potential credential is named by path, and the value never appears in the output.
