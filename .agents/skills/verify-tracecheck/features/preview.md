...
- `preview-gaps` reports omitted files, parse failures (by file and parser error code), budget limits, heuristic-discovery limitations, and TypeScript configuration that could not be used for path aliases. Every file omitted for a potential credential is named by path, and the value never appears in the output. A changed file whose base version holds a potential credential keeps its screened current text, loses its baseline, and is named in a limitation.
