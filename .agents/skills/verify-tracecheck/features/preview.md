...
- `preview-gaps` reports omitted files, parse failures (by file and parser error code), budget limits, and TypeScript configuration that could not be used for path aliases. Every file omitted for a potential credential is named by path, and the value never appears in the output. Caveats that are not gaps, such as heuristic discovery and the count of excluded untracked files, are `notes` (human output: `Note: ...`) and are not part of the snapshot.
