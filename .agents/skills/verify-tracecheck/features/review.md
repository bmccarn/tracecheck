...
Review collects the same scope as preview, sends each nonempty packet to Jev, and returns a report: source-check decisions with status and impact, an independent quality evaluation per packet, limitations, models, and usage. It re-collects after inference and rejects the result if the repository changed. Compare turns two saved reports into a finding history.
