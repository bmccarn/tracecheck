...
- **Previous with several packets.** Use the `multi-packet-constants` fixture: two packets and no candidates. `review --previous` with a single-packet report runs, returns `packetQualities` for both packets, and adds `Previous evaluation was not compared because this review has multiple packet scopes; comparison applies only to a single-packet quality result.` Passing the multi-packet report it saved with `--out` as `--previous` exits `2` with `is a multi-packet review report`.
