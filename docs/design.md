...
Discovery and provider evidence have separate budgets. Import discovery scans eligible tracked JS/TS/Python files by default, with a soft deadline and optional file/byte caps. A bounded in-process cache stores checked file metadata and edges, never raw source; path confinement, inode/device, size, mtime, ctime, and the known path universe govern reuse. The reverse graph is built once rather than scanned for every changed file. Files stream through sixteen workers, so a slow file delays only its own read; the byte budget, cache, and deadline cut are applied in path order, as in a sequential scan. The identity checked by the safe read is compared with the pre-read metadata, so a read needs no second `stat`.

Git evidence uses path-scoped, argument-bounded batches rather than a process per changed file. Current sources pass the safe-read gate before entering a diff. Each changed or supporting file is read and screened once per collection, and packets that share a supporting file reuse that read. Baseline blobs over 8 MiB are omitted before patch generation; accepted baselines stream one at a time into excerpt selection and digesting. NUL-delimited Git identities and literal pathspecs preserve unusual filenames without interpreting source lines as file headers. Renames are detected once with Git's default similarity threshold. A renamed file takes its old path's base version as its baseline, and each detected pair is diffed with rename detection, so changed ranges cover only the edited hunks and a pure rename has none. A rename from an unsupported or generated path is reviewed without a baseline, and that gap is reported. This is collection plumbing, not semantic review logic.
...
Generated paths, external symlinks, unsupported files, parse failures, source with a potential credential, and budget omissions are visible gaps. Every file omitted for a potential credential is named. Credential pattern checks are not comprehensive DLP: they cover private key blocks, common provider token formats, URL passwords, and credential-named assignments whose value does not read as an identifier. Heuristic import and reverse-caller discovery is incomplete; unresolved references and dynamic imports may be missed. Path aliases resolve only through repository tsconfig or jsconfig files; configuration that is external, unreadable, or malformed is reported as a limitation. Selection limits and the graph limitation are reported. Dependency edits do not yet schedule source checks in unchanged callers.
...
The Jev client handles all three primitives, validates requested answer types/options/ranges, retains the resolved model identifier, applies a request deadline across retries, retries rate limits, server errors, and network failures up to three attempts, and honors bounded Retry-After delays. Request and review timeouts report which limit fired and its duration. Responses are limited to 512 KB, and malformed response errors do not echo provider bodies. The same credential screening applies to every string in a request before transmission; a blocked request names the file path or input field and never the matched value. Configurable defaults are 20 seconds for soft import discovery, 120 seconds for collection, and 300 seconds for repository review; cancellation propagates into Git and provider work. CLI's review deadline covers inference and post-review verification; MCP's includes initial recollection too. Errors cannot become successful reviews. Credentials are read from the environment and sent directly to TypeSafe.

Verification and repository review share one request loop. Independent requests run with at most four in flight by default (`JEV_CONCURRENCY` or `requestConcurrency`, from 1 to 16), and their results are applied in plan order, so a report does not depend on which request finishes first. Usage is counted once per completed request. When a request still fails after its retries, the review keeps every completed decision and packet quality, reports `inconclusive`, and adds a `Review incomplete for packet ...` limitation that names the unevaluated source checks and broad review. If no request succeeds, the review fails with the provider's error. Cancellation and deadlines abort every request in flight. Request planning sizes each packet part once, so its cost grows linearly with the number of questions.

Repository reviews cache up to 16 reports for five minutes. Incomplete reports are not cached, so a retry reaches the provider again. Keys include canonical repository identity, source/task/context snapshots, and requested model. Prior evaluations are compared against a cloned report locally and do not invalidate cached inference. The original report time and cache-hit flag remain visible. A mutable model alias can change upstream; choose an available concrete version for reproducible evaluations.
