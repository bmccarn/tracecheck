...
2. Call `tracecheck_preview` with `repo`, `base`, `task`, and optional `repositoryContext` and `collection`. Omitted arguments default to the repository's `.tracecheck.json`, if it has one; an edit to that file between preview and review invalidates the snapshot. Inspect packet membership, `limitations` (coverage gaps), `notes` (caveats that do not affect the status), and the file manifest before review. More packets require more provider calls.
