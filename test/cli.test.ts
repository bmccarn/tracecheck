...
test('preview and review show a task and context from the settings file, labeled as repository-supplied', async t => {
  ...
  assert.deepEqual(JSON.parse((await cli(['preview', '--repo', repo.root, '--json'])).stdout).notes, labels);
  ...
  assert.deepEqual(reportSchema.parse(JSON.parse(json.stdout)).notes, labels);
  ...
  assert.deepEqual(JSON.parse(flagged.stdout).notes, labels.slice(1));
});
